...
def get_quality_share(entities, source):
    rel_exp = entities.get('rel_exp')
    if rel_exp:
        _rel_exp = []
        for exp in rel_exp:
            exp['prop'] = 'quality_share'
            _rel_exp.append(exp)
    ...
    try:
        ...
        if chart_type:
            ...
            data = data.pivot_table(values=['quality_share'], columns=['start_date', 'month'], index=pivot_index, aggfunc=np.sum)
            if data.empty:
...
def get_abs_quality(entities, source):
    rel_exp = entities.get('rel_exp')
    if rel_exp:
        _rel_exp = []
        for exp in rel_exp:
            exp['prop'] = 'abs_quality'
            _rel_exp.append(exp)
        entities['rel_exp'] = _rel_exp
    print(entities)
    columns = ['operator_name']
    ...
    try:
        #if True:
        quality = quality.strip().lower()
        b = table_sniper[['operator_name', 'geo_rgn_name', 'geo_city_name', 'month',
                    'year', 'start_date', 'abs_gross', quality+'_quality_abs']].groupby(columns).sum()
        data = (b[quality+'_quality_abs']/1000).round(1)
        data.name = 'abs_quality'
        ...
        if trend_exp: # query is "trendy", need futher processing.
            ...
            if 'geo_rgn_name' in columns:
                ...
                new_df['difference'] = new_df['abs_quality_max'] - new_df['abs_quality_min']
                _data = new_df[['operator_name', 'geo_rgn_name', 'abs_quality_min', \
                            'abs_quality_max', 'difference']]
                _error_if_empty = 'No such Regions found'
            elif 'geo_city_name' in columns:
                new_df = group_min.merge(group_max, on=['operator_name', 'geo_city_name'], suffixes=['_min', '_max'])
                new_df['difference'] = new_df['abs_quality_max'] - new_df['abs_quality_min']
                _data = new_df[['operator_name', 'geo_city_name', 'abs_quality_min', \
                                'abs_quality_max', 'difference']]
                _error_if_empty = 'No such Cities found'
            else:
                new_df = group_min.merge(group_max, on=['operator_name'], suffixes=['_min', '_max'])
                new_df['difference'] = new_df['abs_quality_max'] - new_df['abs_quality_min']
                _data = new_df[['operator_name', 'abs_quality_min', 'abs_quality_max', 'difference']]
            ...
            if trend_exp['trend'] == 'increase':
                ...
                _title = quality.upper()+'Absolute Quality Increase'
            else:
                _data =  _data[new_df['difference'] < -float(trend_exp['deviation'])]
                _title = quality.upper()+'Absolute Quality Decrease'
            ...
            data.rename(columns={'operator_name': _title,
                                'abs_quality_min': new_df['month_min'][0],
                                'abs_quality_max': new_df['month_max'][0],
            ...
            if chart_type:
                ...
                data = data.round(2)
                #data['quality'] = data['quality'].astype(str) + '%'
                data = data.pivot_table(values=['abs_quality'], columns=['start_date', 'month'], index=pivot_index, aggfunc=np.sum)
                if data.empty:
                    return error_mesg(get_resp_no_records())
                data.columns = data.columns.droplevel([0, 1])
                data = data.reset_index()
        ...
        data.rename(columns={'operator_name': quality.upper()+' Absolute Quality (in 000s)'}, inplace=True)
        data.fillna('-', inplace=True)
        data.replace(to_replace='0.0%', value='-', inplace=True)
        data.columns = beautify_columns(list(data.columns))
        print('res:', data.head())
...
def get_industry_gross_quality(entities, source):
    rel_exp = entities.get('rel_exp')
    if rel_exp:
        _rel_exp = []
        for exp in rel_exp:
    ...
    try:
        ...
        data = (c/1000).round(1)
        data[quality.upper()+' Industry Gross (in 000s)'] = 'Industry'
        data = pd.DataFrame(data).reset_index()
        data.rename(columns={quality+'_quality_abs':'ind_gross'}, inplace=True)
        ...
        if chart_type:
            ...
            data = data.pivot_table(values=['ind_gross'], columns=['start_date', 'month'], index=[quality.upper()+' Industry Gross (in 000s)'], aggfunc=np.sum)
            data.columns = data.columns.droplevel([0, 1])
            data = data.reset_index()
        print(data)
        numerics = ['int16', 'int32', 'int64', 'float16', 'float32', 'float64']
...
def get_net_to_gross(entities, source):
    ...
    try:
        ...
        if "geo_rgn_name" in columns:
            ...
            b = a.sortlevel([0, 1, 4])
            data = b.groupby(level=[0, 1], group_keys=False).apply(
                            lambda x: ((x[kpi_filter+'_base']-x[kpi_filter+'_base'].shift(1))/x[kpi_filter+'_base'])*100)
            data.name = 'net2gross'
            data = pd.DataFrame(data).reset_index()
            ...
            b = a.sortlevel([0, 1, 4])
            data = b.groupby(level=[0, 1], group_keys=False).apply(
                            lambda x: ((x[kpi_filter+'_base']-x[kpi_filter+'_base'].shift(1))/x[kpi_filter+'_gross'])*100)
            data.name = 'net2gross'
            data = pd.DataFrame(data).reset_index()
        else:
            a = table_sniper[cols].groupby(['operator_name', 'month', 'year', 'start_date']).sum()
            b = a.sortlevel([0, 3])
            data = b.groupby(level=[0], group_keys=False).apply(
                            lambda x: ((x[kpi_filter+'_base']-x[kpi_filter+'_base'].shift(1))/x[kpi_filter+'_gross'])*100)
            data.name = 'net2gross'
            data = pd.DataFrame(data).reset_index()
