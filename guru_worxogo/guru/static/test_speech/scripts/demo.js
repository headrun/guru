...
/** Constants **/
   ...
   * @private {!Element|null}
   */
  this.processingCaption_ = document.getElementById('speech_demo_results');
...
cloud.Speech.prototype.sendAudio = function(binaryAudioFile, language, encoding,
  ...
  var request = JSON.stringify({
    config: {
      ...
      maxAlternatives: 1,
      speechContext: {phrases: ["rate of sale", "ROS", "ros", "sell thru", "Arrow", 
                          "Arrow Women", "USPA", "US Polo", "Gant"]}
...
cloud.Speech.prototype.showError = function() {
  ...
  this.isProcessing_ = false;
  //this.setButtonState('');
   this.processingCaption_.placeholder = '';
...
cloud.Speech.prototype.showResults = function(data){
  ...
  if (!data.results) {
   	this.result_.value = 'No speech detected';
    return;
  }
  var transcript = data.results.map(function(result) {
    return result.alternatives[0].transcript;
  });
  this.result_.value = transcript.join('');
};
...
cloud.Speech.prototype.startRecording = function(){
  ...
  //this.setButtonState('recording');
   this.processingCaption_.placeholder = 'listening...';
   this.result_.value = '';
...
cloud.Speech.prototype.stopRecording = function(){
  ...
  //this.setButtonState('processing');
   this.processingCaption_.placeholder = 'processing...';
