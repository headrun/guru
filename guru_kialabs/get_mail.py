...
def text_filters_2(body_text, mail_cc):
        ...
        for _text in body_text.split('\n'):
            ...
            if _text.startswith('On') or ('wrote:' in _text) or (_text.strip() in mail_ids):
                flag += 1
            elif _text.startswith('On') and ('wrote:' in _text) and (_text.strip() in mail_ids):
                flag = 3
            else:
                flag = 0
...
def validate_string(_string, blacklist):
    print '\n', _string
    if not _string:
        return 0
    for _black in blacklist:
        if _black in _string:
           return 1
    return 0
...
def extract_text(body_html):
    ...
    for child in node.childGenerator():
        ...
        if type(child) is bs.NavigableString:
           ...
           print(':::\n\n')
           print(child)
        elif validate_string(child.get('class'), blacklist):
                continue
        else:
           for _ch in child.recursiveChildGenerator():
              if type(_ch) is bs.Tag:
                  if validate_string(_ch.get('class'), blacklist):
                    continue
              else:
                  body_text += ' \n'+child.text
                  print(':::\n\n')
...
def multipart_extract(body):
    ...
    # return values will be unicode strings.
    # dont forget to encode, when printing those.
