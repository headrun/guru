...
chartsDataPath = os.path.join(BASE_DIR, "api/charts_sample.json")
...
URL = 'http://144.76.48.157:8010/search'
  ...
  def parse(self, user, input):
    ...
    for _chunk in result:
        ...
        if _chunk.get('type') == 'message':
            ...
            message = self.createMessage(conv, self.getChart(_chunk['data']), _chunk,
                                    question=question, worth_sending=True)
        if message:
